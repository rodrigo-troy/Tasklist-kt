...
class TasklistTest : StageTest<Any>() {
    ...
    fun tasklistTest07(): CheckResult {
        ...
        val outputStrings = arrayOf(
                ...
                arrayOf("2  Supermarket",
                        "   Chocolates",
                        "   flour",
                        "   oranges"),
                arrayOf("3  Change site",
                        "   Use Christmas theme"),
                arrayOf("4  Buy book"),
                arrayOf("5  Fix printer"),
                arrayOf("6  Cinema: get tickets",
                        "   Check movie reviews"),
    ...
    fun tasklistTest08(): CheckResult {
        ...
        for (s in outputStrings) {
            if (!co.inputNext(*s)) {
                ...
                s.forEach { str.appendLine(it) }
                return CheckResult(false,
                                   "The input lines should be trimmed; your output should contain  \"${str.toString()}\"")
            }
            if (!co.inputNext("\n\n"))
                return CheckResult(false,
                                   "Each task printout should be followed by a blank line")
